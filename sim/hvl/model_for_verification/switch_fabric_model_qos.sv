...
module switch_fabric_model_qos #(
    ...
    qos_frame_t priority_queues [NUM_PORT][8][$];
    int current_qos_level [NUM_PORT];
    ...
    task forward_frame_qos(int src_port, Fabric_frame_tr frame, logic [2:0] qos_tag);
        ...
        for (int dst = 0; dst < NUM_PORT; dst++) begin
            if (frame.dest[dst]) begin
                automatic int qos_idx = int'(qos_tag);
    ...
    task schedule_transmission(int port);
        automatic qos_frame_t selected_pkt;
        ...
        if (qos_enable) begin
            ...
            for (int attempts = 0; attempts < 8; attempts++) begin
                automatic int q = (current_qos_level[port] + attempts) % 8;
                if (priority_queues[port][q].size() > 0) begin
                    selected_pkt = priority_queues[port][q].pop_front();
                    found = 1'b1;
                    current_qos_level[port] = (q + 1) % 8;
                    break;
                end
            end
        end
    ...
    generate
        for (genvar g = 0; g < NUM_PORT; g++) begin : g_tx_drv
            mailbox temp_mailbox_out;

            initial begin
                temp_mailbox_out = new();
                frame_mailbox_out[g] = temp_mailbox_out;
            end
            ...
            fabric_driver #(
                ...
                .frame_mailbox(temp_mailbox_out),
        ...
        for (genvar i = 0; i < NUM_PORT; i++) begin : g_fwd
            initial begin
                ...
                forever begin
                    ...
                    qos_tag = frame_in.id[2:0];  // Simple extraction
    ...
    generate
        for (genvar p = 0; p < NUM_PORT; p++) begin : g_sched
            initial begin
                forever begin
                    automatic bit has_packets = 1'b0;
                    @(posedge clk);
                    ...
                    if (has_packets) schedule_transmission(p);
                end
            end
        end
    endgenerate
