...
module tb_fabric_connectivity;
    ...
    initial begin
        ...
        // Send one packet from each src to each dst
            ...
            initial begin
                ...
                forever begin
                    ...
                    if (tx_data_if[i].valid && tx_data_if[i].ready && tx_data_if[i].last) begin
                        ...
                        // Extract src from packet ID (pkt_id = src*10 + dst + 1)
                        src = (tx_data_if[i].id - 1) / NUM_PORT;
                        dst = i;
                        pkt_idx = src * NUM_PORT + dst;
                        
                        if (src >= 0 && src < NUM_PORT && dst >= 0 && dst < NUM_PORT) begin
    ...
    generate
        for (genvar i = 0; i < NUM_PORT; i++) begin : gen_tx_drv
            ...
            // Metadata driver
            initial begin
                automatic Fabric_frame_tr meta_frame;
                
                rx_meta_if[i].valid = 0;
                rx_meta_if[i].dest_port_mask = 0;
                rx_meta_if[i].id = 0;
                rx_meta_if[i].qos_tag = `PRIORITY_STANDARD;  // Use best-effort
                ...
                forever begin
                    ...
                    if (rx_data_if[i].valid && rx_data_if[i].ready) begin
                        // Send metadata on first beat only
                        if (!rx_meta_if[i].valid) begin
                            rx_meta_if[i].dest_port_mask <= rx_data_if[i].id[3:0];  // Extract from ID
    ...
    task print_connectivity_report();
        ...
        // Print matrix
        $display("║                                                       ║");
        $display("║  Connectivity Matrix (✓ = pass, ✗ = fail):          ║");
        $display("║                                                       ║");
        
        $write("║     ");
        for (int d = 0; d < NUM_PORT; d++) 
            $write("%2d ", d);
        $display("   ║");
        ...
        for (int s = 0; s < NUM_PORT; s++) begin
            ...
            for (int d = 0; d < NUM_PORT; d++) begin
                if (connectivity_matrix[s][d]) begin
                    ...
                    // Check latency
                    automatic int idx = s * NUM_PORT + d;
                    automatic real lat = (packet_rx_time[idx] - packet_tx_time[idx]) / 1000.0;
                    avg_latency += lat;
                    if (lat > 1.0) latency_violations++;
                end else begin
                    $write(" ✗ ");
                    failed++;
                end
