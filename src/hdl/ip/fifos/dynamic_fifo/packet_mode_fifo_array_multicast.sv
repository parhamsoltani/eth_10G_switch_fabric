...
// Module Name: packet_mode_fifo_array_multicast
// Project Name: 
// Target Devices: 
...
module packet_mode_fifo_array_multicast #(
    ...
    localparam NP_WIDTH                     = NUM_IN_LOG + MAIN_MEM_DEPTH_LOG + 1 + META_DATA_WIDTH +  NUM_FIFO_LOG;
    ...
    localparam MASK_FIFO_DEPTH = 32;


    // addr_fifos input wires
    wire                                addr_fifos_push;
    ...
    reg                                 mask2index_push;
    wire [NUM_FIFO-1:0]                 mask2index_mask;
    wire [MAIN_MEM_DEPTH_LOG-1:0]       mask2index_data_in;
    wire                                mask2index_push_out;
    wire [NUM_FIFO_LOG-1:0]             mask2index_index;
    wire [MAIN_MEM_DEPTH_LOG-1:0]       mask2index_data_out;
    wire                                mask2index_prog_full;


    wire [NUM_FIFO_LOG:0]               num_non_zero_mask_in;


    wire                                num_pop_mem_wr_en;
    wire [MAIN_MEM_DEPTH_LOG-1:0]       num_pop_mem_wr_addr;
    wire [NUM_FIFO_LOG-1:0]               num_pop_mem_wr_data;

    wire                                num_pop_mem_rd_en;
    wire [MAIN_MEM_DEPTH_LOG-1:0]       num_pop_mem_rd_addr;
    wire [NUM_FIFO_LOG-1:0]               num_pop_mem_rd_data;
    ...
    wire                                pop_last_D [0:1];
    wire [NUM_FIFO-1:0]                 push_output_id_D [0:1];
    wire [NUM_FIFO_LOG-1:0]             num_pop_D [0:1];


    wire [NUM_FIFO_LOG-1:0]               num_push;
    wire [NUM_FIFO_LOG-1:0]               num_pop;

    wire [NUM_FIFO_LOG-1:0]               num_push_write;
    ...
    assign addr_fifos_push       = mask2index_push_out;
    assign addr_fifos_push_data  = mask2index_data_out;
    assign addr_fifos_push_id    = mask2index_index;
    assign addr_fifos_pop        = pop_D[1] && prev_last;
    assign addr_fifos_pop_id     = pop_id_D[1];

    // free address fifo input signals

    assign free_fifo_push = pop_D[5] && (num_push == num_pop);
    assign free_fifo_push_data  = rd_addr_out_reg_D[1];
    assign free_fifo_pop        = push_i;

    // cancat np and last and metadata mem input signals
    assign num_push_write = num_non_zero_mask_in - 1;

    assign np_wr_en    = push_D[1];
    assign np_wr_addr  = push_tp;
    assign np_wr_data  = {num_push_write, push_input_id_D[1], free_fifo_pop_data_D[1], push_last_D[1], push_meta_data_D[1]};
    ...
    assign out_addr_mem_wr_data  = {np_rd_data[NP_WIDTH - (NUM_FIFO_LOG)-NUM_IN_LOG-1:META_DATA_WIDTH + 1], np_rd_data[META_DATA_WIDTH]};
    assign out_addr_mem_rd_en    = pop_i;
    assign out_addr_mem_rd_addr  = pop_id_i;


    assign num_pop_mem_wr_en    = pop_D[5];
    assign num_pop_mem_wr_addr  = rd_addr_out_reg_D[1];
    assign num_pop_mem_wr_data = (num_push == num_pop) ? 0 : num_pop + 1;
    assign num_pop_mem_rd_en    = pop_D[4];
    assign num_pop_mem_rd_addr  = rd_addr_out_reg_D[0];
    ...
    always @(posedge clk) begin
        mask2index_push <= push_i && sof[push_input_id_i];
    end
    assign mask2index_mask     = push_output_id_D[1];
    assign mask2index_data_in  = push_hp;


    assign num_push             = np_rd_data[NP_WIDTH-1:NP_WIDTH-NUM_FIFO_LOG];
    assign num_pop              = ((rd_addr_out_reg_D[1] == rd_addr_out_reg_D[2]) && pop_D[6]) ? num_pop_D[1] + 1 : num_pop_mem_rd_data;
    ...
    assign pop_input_id_o   = np_rd_data[NP_WIDTH - (NUM_FIFO_LOG)-1:NP_WIDTH - (NUM_FIFO_LOG)-NUM_IN_LOG];
    ...
    always @(posedge clk) begin
        ...
        ready_reg <= ready_from_free_fifo && ready_from_addr_fifo && (!mask2index_prog_full);
    end
    ...
    multicast_mask_to_index #(
        .N                 (NUM_FIFO),
        .DATA_WIDTH        (MAIN_MEM_DEPTH_LOG),
        .MASK_FIFO_DEPTH   (MASK_FIFO_DEPTH)
    ) mask2index (
        .clk            (clk),
        .push_i        (mask2index_push),
        .mask_i        (mask2index_mask),
        .data_i        (mask2index_data_in),
        .push_o        (mask2index_push_out),
        .index_o       (mask2index_index),
        .data_o        (mask2index_data_out),
        .prog_full_o   (mask2index_prog_full)
    );
    ...
    delayed_regs #(
        .WIDTH      (NUM_FIFO),
        .NUM_DELAY  (1)
    ) push_output_id_delay_inst (
        .clk            (clk),
        .signal_in      (push_output_id_i),
        .delayed_signal (push_output_id_D)
    );
    ...
    sdpram_init_value_all_same #(
        ...
        .clk        (clk),
        .wr_en_i    (num_pop_mem_wr_en),
        .wr_addr_i  (num_pop_mem_wr_addr),
        .wr_data_i  (num_pop_mem_wr_data),
        .rd_en_i    (num_pop_mem_rd_en),
        .rd_addr_i  (num_pop_mem_rd_addr),
        .rd_data_o  (num_pop_mem_rd_data)
    );
