...
L_MIN         = 64          # Used only when CELL_SWITCH == False
GAP_MIN       = 13           # Used only when CELL_SWITCH == False

# Part number control
FORCE_PART_NUMBER = "xcvu9p-flga2577-3-e"      # e.g. "xcvu9p-flga2577-3-e" or None to auto-pick by N
...
def pick_part_number(n_val: int, forced: str | None) -> str:
...
def main():
    ...
    for i, dvals in enumerate(define_dicts, start=1):
        ...
        part = pick_part_number(N_int, FORCE_PART_NUMBER)
